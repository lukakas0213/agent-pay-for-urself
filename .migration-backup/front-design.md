...
이번 재설계의 목표 화면 구조는 아래 5개로 고정한다.

* `대시보드`: 메인 에이전트와 ChatGPT처럼 대화하는 화면만 둔다.
* `에이전트`: 메인 에이전트와 각 서브 에이전트의 프롬프트, 에이전트별 특화 설정을 수정한다.
* `히스토리 타임라인`: 메인 에이전트와 서브 에이전트가 어떤 순서와 상태로 일했는지 타임라인으로 본다.
* `보고서`: 저장된 보고서를 날짜순으로 리스트업하고, 클릭하면 상세 내용을 본다.
* `계좌`: 계좌 연결 입력과 연결된 계좌 상태 확인을 제공한다.

### Current Gap Summary

현재 구현과 이번 목표 구조의 차이는 아래와 같다.

* 현재 `대시보드`는 채팅 전용이 아니라 실행 폼, 최근 런 카드, 에이전트 요약 카드, 보고서 저장 패널까지 함께 둔다.
* 현재 `에이전트` 화면은 서브 에이전트 중심이며 메인 에이전트 전용 진입점과 에이전트별 설정 계층이 없다.
* 현재 `히스토리 타임라인`은 전용 화면과 전용 API가 없다. 일부 로컬 상태와 저장 실행만 있다.
* 현재 `보고서`는 저장 보고서 목록과 상세 복기는 가능하지만, 날짜순 리스트 계약을 더 명확히 유지해야 한다.
* 현재 `계좌 연결`은 브라우저 로컬 입력에 머물고, Toss API와 한국투자 API 연결 정보를 서버 계약으로 저장하거나 검증하지 않는다.
...
* `히스토리` 그룹은 기본 상태에서 전체 요약 히스토리와 각 에이전트별 히스토리를 함께 보여준다.
...
대시보드는 메인 에이전트와 직접 소통하는 채팅 허브다.

필수 구성:
* 메인 에이전트와 대화하는 단일 주 작업 화면
* 채팅 입력창과 메시지 리스트
* 기존 실행 결과를 기반으로 하는 follow-up 질문
* 필요 시 같은 채팅 흐름에서 워크플로우 재실행
* 현재 run 맥락, 마지막 실행 요약, watch 심볼 정도만 보조 정보로 유지

제외:
* 별도 실행 입력 폼을 메인 패널에 크게 두지 않는다.
* 최근 런 카드 그리드, 즐겨찾는 에이전트 카드, 워크플로우 그룹 프리뷰를 대시보드 핵심 구조로 두지 않는다.
* 보고서 저장 패널을 대시보드의 주 시각 구조로 두지 않는다.
...
* 메인 에이전트 이벤트와 서브 에이전트 이벤트를 같은 시간축에서 연결
...
* 계좌 연결 진입점
  * 연결할 Toss API 정보 입력
  * 연결할 한국투자증권 API 정보 입력
  * 어떤 브로커를 현재 활성 연결로 볼지 선택 또는 확인
* 계좌 상태 확인 진입점
...
대시보드는 `Main Agent Chat` 중심 콘솔로 설계한다.

### Section 1. Conversation Thread

* 메인 에이전트와의 대화 메시지 리스트
* 사용자 메시지와 에이전트 응답을 분리
* 현재 run 기준 follow-up 질문과 재실행 응답을 같은 흐름에 쌓는다
* 응답 안에는 최소한 아래가 드러나야 한다
  * 메인 요약
  * 현재 판단 요약
  * 필요한 경우 다음 액션 제안
...
규칙:
* 첫 실행 전에는 새 워크플로우 입력으로 동작한다.
* run_id가 생긴 뒤에는 follow-up 질문 또는 재실행 지시로 동작한다.

### Section 3. Run Context Rail

채팅 오른쪽 또는 상단 보조 패널에 아래를 compact하게 둔다.
...
* agent detail은 prompt editor + agent-specific settings + latest output cards
* 에이전트별 실행 출력과 상태를 확인 가능해야 함
* 메인 에이전트도 서브 에이전트와 같은 깊이의 설정 진입점을 가진다
...
* 저장된 run과 아직 저장되지 않은 최근 run을 함께 열 수 있어야 한다
...
현재 목표 구조 기준으로는 전용 타임라인 endpoint가 필요하다.
...
목표 구조에서 추가로 필요한 backend contract:
* 브로커 연결 정보 저장/조회 endpoint
* 연결 대상 broker kind 구분
* Toss API 연결 상태
* 한국투자 API 연결 상태
...
* Toss API와 한국투자 API 입력값을 브라우저 로컬 상태에만 두는 것으로 끝내지 않는다.
...
현재 gap:
* 메인 에이전트 prompt 항목은 프롬프트 병합 계층에 존재하지만, 기본 prompt 목록과 프론트 타입에는 충분히 노출되지 않는다.
* 에이전트별 특화 설정을 저장하는 공개 API가 없다.
...
추가로 필요한 backend contract:
* `main_agent`를 포함한 프롬프트 목록 표준화
* 에이전트별 설정 저장/조회 endpoint
* 에이전트 설정 스키마 정의
