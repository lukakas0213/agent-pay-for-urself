          ...
          <nav className="sidebar-group" aria-label="Primary navigation">
            ...
            <div className="sidebar-list">
              {primaryNavItems.map((item) => {
                ...
                return (
                  <Link className={`sidebar-link ${active ? "sidebar-link-active" : ""}`} href={item.href} key={item.href}>
          ...
          <div className="sidebar-group">
            ...
            <div className="sidebar-list">
              {agentDefinitions.map((item) => {
                ...
                return (
                  <Link className={`sidebar-link ${active ? "sidebar-link-active" : ""}`} href={item.path} key={item.key}>
                    <span className="sidebar-icon" aria-hidden="true">{sectionIcon(item.label)}</span>
                    <span className="sidebar-label">{item.label}</span>
                  </Link>
                );
          ...
          <div className="sidebar-group sidebar-group-resources">
            ...
            <div className="sidebar-list sidebar-list-dense">
              {resourceItems.map((item) => {
                const active = isActivePath(pathname, item.href);
                return (
                  <Link className={`sidebar-link sidebar-link-resource ${active ? "sidebar-link-active" : ""}`} href={item.href} key={item.label}>
                    <span className="sidebar-label">{item.label}</span>
