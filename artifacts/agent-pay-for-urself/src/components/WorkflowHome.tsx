  ...
  const favoriteAgentCards = agentDefinitions.map((agent) => {
    ...
    if (agent.key === "buy_sell") {
      return {
        key: agent.key,
        label: agent.label,
        meta: `${activeRun.decisions.length}개 판단`,
        body: activeRun.decisions[0]
          ? `${activeRun.decisions[0].symbol} ${actionLabel(activeRun.decisions[0].action)} · ${formatScore(activeRun.decisions[0].confidence)}`
          : "판단 결과 없음",
      };
    }
    if (agent.key === "order_execution") {
      return {
        key: agent.key,
        label: agent.label,
        meta: `${activeRun.orders.length}개 주문 계획`,
        body: activeRun.orders[0]
          ? `${activeRun.orders[0].symbol} ${activeRun.orders[0].should_submit ? "제출 가능" : "차단"} · ${activeRun.orders[0].reason}`
          : "주문 계획 없음",
      };
    }
    return {
      key: agent.key,
      label: agent.label,
      meta: `${activeRun.evaluation_log.decision_count}개 판단`,
      body: activeRun.evaluation_log.notes.join(" / ") || "후속 메모 없음",
    };
  });
  ...
  const workflowGroups = [
    {
      ...
      meta: activeRun
        ? `${activeRun.market_data.length} / ${activeRun.analysis_signals.length} / ${activeRun.investment_reports.length}`
        : "아직 실행 없음",
    ...
    },
    {
      ...
      meta: activeRun
        ? `${activeRun.decisions.length}개 판단 / ${activeRun.orders.length}개 주문`
        : "아직 실행 없음",
    ...
    },
    {
      ...
      meta: activeRun
        ? `${activeRun.mandate_violations.length}개 위반 / ${activeRun.evaluation_log.blocked_order_count}개 차단`
        : "아직 실행 없음",
      ...
      <section className="discover-section">
        ...
        {recentRuns.length ? (
          ...
          <div className="empty-panel">
            <h3>최근 실행 기록이 없습니다</h3>
            <p>새 실행을 시작하면 이 섹션에 카드가 생성됩니다.</p>
          </div>
        )}
      </section>
      ...
      <section className="discover-section">
        ...
        <div className="discover-card-grid discover-card-grid-flow">
          {workflowGroups.map((group) => (
            <article className="discover-card" key={group.title}>
              <div className="discover-card-top">
                <div>
      ...
      <section className="discover-control-grid">
        <article className="discover-panel">
          ...
          <form className="discover-form" onSubmit={handleRun}>
            <div className="discover-form-grid discover-form-grid-3">
              ...
              <label>
                <span>최대 비중</span>
                <input value={maxPositionWeight} onChange={(event) => setMaxPositionWeight(event.target.value)} />
              </label>
              <label>
                <span>리스크</span>
            ...
            <label>
              <span>메인 목표</span>
              <textarea rows={4} value={userPrompt} onChange={(event) => setUserPrompt(event.target.value)} />
            </label>
            ...
            <label>
              <span>추가 메모</span>
              <textarea rows={3} value={userNotes} onChange={(event) => setUserNotes(event.target.value)} />
            </label>
            <div className="discover-toggle-row">
              <label>
                <input checked={autoTradingEnabled} onChange={(event) => setAutoTradingEnabled(event.target.checked)} type="checkbox" />
                자동매매 승인 기본값
              </label>
              <label>
                <input checked={applyChatToWorkflow} onChange={(event) => setApplyChatToWorkflow(event.target.checked)} type="checkbox" />
                후속 대화 재실행 반영
              </label>
            </div>
            <button className="discover-primary-button" disabled={!canRun} type="submit">
              {isLoading ? "실행 중..." : "실행"}
            </button>
          </form>
        </article>
        ...
        <article className="discover-panel">
          ...
          <form className="discover-form" onSubmit={handleFollowUp}>
            <label>
              <span>후속 질문</span>
              <textarea rows={5} value={chatDraft} onChange={(event) => setChatDraft(event.target.value)} />
            </label>
            <button
              className="discover-secondary-button"
              disabled={!activeRun?.run_id || !chatDraft.trim() || isChatLoading}
              type="submit"
            >
              {isChatLoading ? "전송 중..." : "질문 보내기"}
            </button>
          </form>
          <div className="discover-chat-list">
            {chatEntries.slice(-4).map((entry) => (
              <article className="discover-chat-item" key={entry.id}>
                <div>
                  <strong>{entry.role === "user" ? "사용자" : entry.role === "assistant" ? "어시스턴트" : "시스템"}</strong>
        ...
        <article className="discover-panel">
          ...
          <form className="discover-form" onSubmit={handleSaveReport}>
            ...
            <label>
              <span>설명</span>
              <textarea rows={5} value={saveDescription} onChange={(event) => setSaveDescription(event.target.value)} />
            </label>
            <button className="discover-secondary-button" disabled={!canSave} type="submit">
              {isSaving ? "저장 중..." : "저장"}
            </button>
          </form>
        </article>
        ...
        <article className="discover-panel">
          ...
          {activeRun ? (
            <div className="active-summary-list">
              <div>
                <span>메인 요약</span>
                <strong>{buildRunSummary(activeRun)}</strong>
              </div>
              <div>
                <span>판단</span>
                <strong>
                  {activeRun.decisions.map((item) => `${item.symbol} ${actionLabel(item.action)}`).join(" / ") || "없음"}
                </strong>
              </div>
              <div>
                <span>주문 계획</span>
                <strong>
                  {activeRun.orders.map((item) => `${item.symbol} ${item.should_submit ? "submit" : "blocked"}`).join(" / ") || "없음"}
                </strong>
              </div>
              <div>
                <span>평가 메모</span>
                <strong>{activeRun.evaluation_log.notes.join(" / ") || "없음"}</strong>
              </div>
            </div>
          ) : (
            <div className="empty-panel">
              <h3>선택된 실행 없음</h3>
              <p>실행 후 요약이 여기에 표시됩니다.</p>
            </div>
          )}
        </article>
        ...
        <article className="discover-panel">
          ...
          {activeRun ? (
            <div className="resource-mini-grid">
              <div>
                <span>데이터 수집</span>
                <strong>{activeRun.market_data.length}</strong>
              </div>
              <div>
                <span>데이터 분석</span>
                <strong>{activeRun.analysis_signals.length}</strong>
              </div>
              <div>
                <span>보고서 작성</span>
                <strong>{activeRun.investment_reports.length}</strong>
              </div>
              <div>
                <span>주문 실행</span>
                <strong>{activeRun.orders.length}</strong>
              </div>
            </div>
          ) : (
            <div className="empty-panel">
              <h3>런 결과 없음</h3>
              <p>실행 후 리소스 카운트가 표시됩니다.</p>
            </div>
          )}
        </article>
      </section>
